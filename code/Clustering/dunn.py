...
def dunn_index(X, labels, metric):
    """
...
def min_cluster_distances(labels, distances):
    ...
    for i in np.arange(0, len(labels) - 1):
        for ii in np.arange(i + 1, len(labels)):
            if labels[i] != labels[ii] and (distances[i, ii] < min_distances[labels[i], labels[ii]] or min_distances[labels[i], labels[ii]]==0):
...
if __name__ == '__main__':
    ...
    folder = file_folder + '/results/clusters/all_genes_tcga_OV/'
    
    c = np.load(folder + 'algo0/clusters_penalty_2.0_distance_13.npy')
    
    dist = np.load(file_folder + '/results/distance_all_genes_tcga_OVmethod13.pkl.npy')
