...
To compile:
g++ density_decomposition.cpp -fopenmp -fpermissive -I ./boost_1_63_0/ -o density_decomposition -O3

To execute:
./density_decomposition ncpu iter net.txt rates.txt pavafit.txt cuts.txt exact.txt
...
typedef struct {
	unsigned s;
	unsigned t;
  double a;//alpha value of edge (s,t) in s
  double w;//weight of edge (s,t)
} edge;
...
optim* readedgelist(char* edgelist){
	...
	while (fscanf(file,"%u %u %f", &(opt->edges[opt->e].s), &(opt->edges[opt->e].t), &(opt->edges[opt->e].w))==3) {
...
//Step 1: Frank-Wolf gradient descent
...
void init(optim *opt){
  ...
  for (unsigned k=0;k<opt->n;k++){
	...
	for (unsigned k=0;k<opt->e;k++){
        opt->edges[k].a=opt->edges[k].w * .5;
		opt->nodes[opt->edges[k].s].r+=opt->edges[k].w * .5;
		opt->nodes[opt->edges[k].t].r+=opt->edges[k].w * .5;
	}
}
...
void onepass(optim *opt){
	...
	for (k=0;k<opt->e;k++){//parfor
		...
		if (opt->nodes[i].r<opt->nodes[j].r){
			...
			opt->nodes[i].r+=gamma*(opt->edges[k].w-opt->edges[k].a);//carefull
			#pragma omp atomic update
			opt->nodes[j].r-=gamma*(opt->edges[k].w -opt->edges[k].a);
			opt->edges[k].a=(1.-gamma)*opt->edges[k].a+ opt->edges[k].w * gamma;
		}
...
void prepava(optim *opt){
  ...
  //rank source and target of nodes to have the source with lower r
  for (unsigned i=0;i<opt->e;i++){
		...
		if (u<v){
			...
			opt->edges[i].s=v;
			opt->edges[i].t=u;
			opt->edges[i].a=opt->edges[i].w-opt->edges[i].a;
		}
	}
  //free(newlabel);
  qsort(opt->edges,opt->e,sizeof(edge),compare_edges);
    //compute unweighted degree per node
	opt->cd=(unsigned*)calloc((opt->n+1),sizeof(unsigned));
	for (unsigned i=0;i<opt->e;i++){
		opt->cd[opt->edges[i].s+1]++;
	}
	//compute cumulative degree per node (ranked)
	for (unsigned i=0;i<opt->n;i++){
		opt->cd[i+1]+=opt->cd[i];
	}
  //compute ne: nb edges from u to nodes before
...
isoreg* mkcut(isoreg* fit, optim *opt){
	...
	j1=0;
	j2=0;
	//for each aggregated  point, we check its components, and check edges outside the aggregated set
	for (unsigned i=0;i<fit->n;i++){
...
subgraph **mkindep(optim* opt,unsigned *nsub){
	*nsub=opt->cuts[opt->n-1]+1;
	...
	//build sets of subgraphs, find the nodes
	j=0;
	for (unsigned k=0;k<opt->n;k++){
		sgs[opt->cuts[k]]->n++;
		newlabel[k]=j++;
	...
	//build sets of subgraphs, find the corresponding edges
	for (unsigned k=0;k<opt->e;k++){
		u=opt->edges[k].s;
		v=opt->edges[k].t;
		p=opt->cuts[v];
	...
	//build sets of subgraphs, initialize the structures
	for (unsigned k=0;k<*nsub;k++){
	...
	//build edge structures for same cuts, in loops edges between different subgraphs
	for (unsigned k=0;k<opt->e;k++){
		u=opt->edges[k].s;
		v=opt->edges[k].t;
		u2=newlabel[u];
		...
		if (opt->cuts[u]==p){
			...
			sgs[p]->deg[u2]+=sgs[p]->edges[sgs[p]->e].w;
			sgs[p]->deg[v2]+=sgs[p]->edges[sgs[p]->e++].w;
		}
		else{
			sgs[p]->loops[v2]+=sgs[p]->edges[sgs[p]->e].w;
			sgs[p]->l++;
		}
	}
	return sgs;
...
//to run maxkflow in each subgraph
...
void ReReadGraph(Traits::vertex_descriptor &s, Traits::vertex_descriptor &t, subgraph *sg, Graph &g) {
	...
	double capacity1=2*alpha, capacity2;
	...
	for(unsigned i=0; i<sg->e; i++){
		...
		capacity2=sg->edges[i].w;
		AddEdge(u,v,rev, capacity2, g);
		AddEdge(v,u,rev, capacity2, g);
	}
}
	...
	if (n_nodes>0){
		...
		for(unsigned i=0;i<sg->e;i++){
			if (isin[sg->edges[i].s] && isin[sg->edges[i].t]){
				...
				sg1->edges[j1].w=newlabel[sg->edges[i].w];
				sg1->deg[sg1->edges[j1].s]+=sg1->edges[j1].w;
				sg1->deg[sg1->edges[j1].t]+=sg1->edges[j1].w;
				j1++;
			}
				...
				sg2->edges[j2].w=newlabel[sg->edges[i].w];
				sg2->deg[sg2->edges[j2].s]+=sg->edges[i].w;
				sg2->deg[sg2->edges[j2].t]+=sg->edges[i].w;
				j2++;
			}
		}
		sg1->e=j1;
...
int main(int argc,char** argv){
  ...
  printf("- Step 1: Frank-Wolf gradient descent (%u iterations)\n",rep);
	t1=time(NULL);
	init(opt);
