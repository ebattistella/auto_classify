...
//The following structure is used to check adjacency between two nodes
typedef struct {
	unsigned n;//number of nodes
...
//Printing results
void printres(treenode* trn,edgelist *el,char* output) {
	unsigned i=0;
	FILE *file=fopen(output,"w");
	fprintf(file,"val: %.10le\n",trn->val);
	fprintf(file,"size: %u\n",trn->size);
	fprintf(file,"nodes:");
	for (i=0;i<trn->size;i++){
		fprintf(file," %u",el->map[trn->nodes[i]]);
	}
	fclose(file);
}
